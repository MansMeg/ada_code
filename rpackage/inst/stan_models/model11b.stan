...
transformed data {
  ...
  int<lower=0, upper=1> use_latent_state_v2 = 0;
  real<lower=0> gs[N];

  if(use_latent_state_version == 2)
    use_latent_state_v2 = 1;
...
parameters {
  ...
  real<lower=0> sigma_xc[use_latent_state_v2 ? P : 0];
...
model {
   ...
   vector[P] tau_x_mod;
    ...
    for(t in t_start[1]:t_end[1]){
      ...
      Sigma_x = quad_form_diag(Omega_x, tau_x_mod);
      
      
      if(use_latent_state_version == 1){
       for (p in 1:P)
        Sigma_x[p, p] = Sigma_x[p, p] + 1e-6;
      }
      if(use_latent_state_version == 2){
        for (p in 1:P)
          Sigma_x[p, p] = Sigma_x[p, p] +  sigma_xc[p];
      }
