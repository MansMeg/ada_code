...
parameters {
  ...
  corr_matrix[use_multivariate_model ? P : 0] Omega_x[use_multivariate_model ? no_Omega : 0]; // correlation matrix
}
...
model {
  ...
  if(use_multivariate_version > 0)
    for(i in 1:no_Omega)
      target += lkj_corr_lpdf(Omega[i] | nu_lkj);
  ...
  for(t in t_start_all:t_end_all){
    ...
    if(use_softmax){
      target += multi_normal_lpdf(eta[t, ] | eta[t-1, ], quad_form_diag(Omega[s_t_Omega[t]], sigma_latent));
    } else {
      target += multi_normal_lpdf(x[t, ] | x[t-1, ], quad_form_diag(Omega[s_t_Omega[t]], sigma_latent));
    }
