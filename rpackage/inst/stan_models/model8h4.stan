...
transformed parameters {
  ...
  if(use_house_bias){
    ...
    for(p in 1:P) {
      for(h in 1:H) {
        beta_mu[1, h, p] = beta_mu_z[1, h, p] * beta_mu_1_sigma_hyper;
        // target += normal_lpdf(beta_mu[1,h,p] | 0, beta_mu_1_sigma_hyper);
        if(S > 1){
          for(s in 2:S) {
            beta_mu[s, h, p] = alpha_beta_mu[1] * beta_mu[s - 1, h, p] + beta_mu_z[s, h, p] * sigma_beta_mu[1];
...
//    equivalent to: target += multi_normal_cholesky_lpdf(eta[t, ] | eta[t-1, ], diag_pre_multiply(sigma_latent, L_Omega[s_t_Omega[t]]));
    ...
    for(t in t_start_all:t_end_all){
      target += multi_normal_cholesky_lpdf(x[t, ] | x[t-1, ], diag_pre_multiply(sigma_x, L_Omega[s_t_Omega[t]]));
    }
  }
...
//  }
  ...
  if(use_house_bias){
    for(p in 1:P){
      for(h in 1:H) {
        target += std_normal_lpdf(beta_mu_z[1, h, p]);
        // target += normal_lpdf(beta_mu[1,h,p] | 0, beta_mu_1_sigma_hyper);
        if(S > 1){
          for(s in 2:S) {
