...
transformed parameters {
  ...
  vector<lower=2>[use_t_dist_industry_bias ? 1 : 0] nu_kappa = rep_vector(2, use_t_dist_industry_bias ? 1 : 0);
  ...
  if(use_industry_bias){
    ...
    if(use_t_dist_industry_bias){
      nu_kappa[1] = nu_kappa_raw[1] + 2.0;
    }
    for(p in 1:P){
      // non-centering
      if(use_ar_kappa){
        if(use_t_dist_industry_bias){
          kappa[1, p] = sqrt(nu_kappa[1] / v_kappa[1]) * kappa_raw[1, p] * kappa_1_sigma_hyper;
          for(j in 2:no_unknown_kappa)
            kappa[j, p] = alpha_kappa[1] * kappa[j-1, p] + sqrt(nu_kappa[1] / v_kappa[1]) * kappa_raw[j, p] * sigma_kappa[p];
        } else {
        ...
        if(use_t_dist_industry_bias){
          for(j in 1:no_unknown_kappa)
             kappa[j,p] = sqrt(nu_kappa[1] / v_kappa[1]) * kappa_raw[j, p] * sigma_kappa[p];
        } else {
          for(j in 1:no_unknown_kappa)
...
model {
  ...
  if(use_industry_bias){
    if(use_t_dist_industry_bias){
      ...
      target += chi_square_lpdf(v_kappa | nu_kappa);
    }
    for(p in 1:P){
      for(j in 1:no_unknown_kappa) {
...
generated quantities{
  ...
  if(estimate_kappa_next == 0){
    ...
    if(estimate_alpha_kappa){
      if(use_t_dist_industry_bias){
        for(p in 1:P)
          kappa_pred[(T_known + 1), p] = student_t_rng(nu_kappa[1], alpha_kappa[1] * kappa[no_unknown_kappa, p], sigma_kappa[p]);
      } else {
        for(p in 1:P)
      ...
      if(use_t_dist_industry_bias){
        for(p in 1:P)
          kappa_pred[(T_known + 1), p] = student_t_rng(nu_kappa[1], 0, sigma_kappa[p]);
      } else {
        for(p in 1:P)
