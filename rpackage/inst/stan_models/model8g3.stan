...
parameters {
  matrix<lower=0, upper=1>[use_softmax ? 0 : T - T_known, use_softmax ? 0 : Px] x_z_unknown; // unknown states (proportions)
...
transformed parameters {
  ...
  if(use_softmax){
    ...
    // print(eta_z);
    // print(sigma_x);
    // print(L_Omega[s_t_Omega[1:2]]);
    eta[1,] = to_row_vector(diag_pre_multiply(sigma_x, L_Omega[s_t_Omega[1]]) * to_vector(eta_z[1,]));
    for(t in t_start_all:t_end_all){
      if(x_t_is_known[t]){ // then eta_z_t is also known (see derivation)
        ...
        eta[t,] = eta[t-1,] + to_row_vector(diag_pre_multiply(sigma_x, L_Omega[s_t_Omega[t]]) * to_vector(eta_z[t,]));
      }
    }
    ...
    x_z[x_unknown_t, ] = x_z_unknown;
    x[x_known_t, ] = x_known;
    x[1,] = to_row_vector(diag_pre_multiply(sigma_x, L_Omega[s_t_Omega[1]]) * to_vector(x_z[1,]));
    for(t in t_start_all:t_end_all){
      if(x_t_is_known[t]){ // then x_z_t is also known (see derivation)
        x_z[t,] = to_row_vector(inverse(diag_pre_multiply(sigma_x, L_Omega[s_t_Omega[t]])) * to_vector((x[t,] - x[t-1,])));
      } else {
        x[t,] = x[t-1,] + to_row_vector(diag_pre_multiply(sigma_x, L_Omega[s_t_Omega[t]]) * to_vector(x_z[t,]));
      }
    }
  }
