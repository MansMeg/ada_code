...
data {
  ...
  int<lower=1> g[N]; // time points since last known state: how many time steps are the poll (mid point) from the previous election divided by the total
...
parameters {
  ...
  matrix[T_known, P] kappa_unknown; // Industry bias
  real <lower=0> sigma_kappa; // Industry bias effect
}
...
transformed parameters {
  ...
  for(p in 1:P) {
    for(t_known in 1:T_known) {
      kappa[t_known, p] = kappa_unknown[t_known, p];
    }
    kappa[T_known + 1, p] = 0.0;
  }
...
model {
  ...
  for(p in 1:P){
    for(j in 1:(T_known+1)) {
       target += normal_lpdf(kappa[j, p] | 0 , sigma_kappa);
    }
  }
  target += normal_lpdf(sigma_kappa | 0, sigma_kappa_hyper);
