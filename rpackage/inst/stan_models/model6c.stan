...
transformed data {
  ...
  real<lower=0, upper=1> sigma_x_hyper = sqrt(time_scale_length / 30);
}
...
transformed parameters {
  ...
  matrix<lower=0>[T, P] sigma_x_t = rep_matrix(0, T, P);
  ...
  // Let the dynamic component vary with mu
  for(p in 1:P){
    for(t in t_start[p]:t_end[p]) {
      sigma_x_t[t,p] = sqrt(x[t,p] * (1 - x[t,p])) * sigma_x[p];
    }
  }
...
model {
  ...
  for(p in 1:P){
    for(t in t_start[p]:t_end[p]) { //
      ...
      target += normal_lpdf(x[t, p] | x[t-1, p], sigma_x_t[t, p]);
    }
  }
