...
parameters {
  ...
  real<lower=0,upper=1> theta_x; // dynamic movement
...
transformed parameters {
  ...
  if(use_industry_bias){
    for(p in 1:P){
      for(i in 1:N) {
        if(y_missing[i, p] == 0){
            mu[i,p] = mu[i,p] + g[i] * kappa[next_known_state_index[i], p];
          }
        }
      }
    }
  ...
  for(p in 1:P){
    ...
    for(t in t_start[p]:t_end[p]) { 
      V[t, p ] =  V_noise[t, p] + alpha_V * V[t-1,p];
    }
  }
}
...
model {
  ...
  //target += normal_lpdf(alpha_x     | 3, 4);
  target += beta_lpdf(theta_x| 2, 2);
  // latent state
  for(p in 1:P){
    // TODO: Add priors
    target += inv_gamma_lpdf(V_noise[ t_start[p]-1, p]| 2, 1 );
    for(t in t_start[p]:t_end[p]) { 
      target += inv_gamma_lpdf(V_noise[t, p]| 2, 1 );
      target += normal_lpdf(x[t, p] | x[t-1, p], sigma_x[p] * ((1-theta_x)  * V[t,p]  + theta_x) );
    }
  }
