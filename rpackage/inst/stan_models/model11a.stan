...
parameters {
  ...
  corr_matrix[P] Omega_x;        // prior correlation
  vector<lower=0>[P] tau_x;      // prior scale
}
...
model {
  ...
  tau_x ~ cauchy(0, 2.5);
  Omega_x ~ lkj_corr(2);
  Sigma_x = quad_form_diag(Omega_x, tau_x);
    for(t in t_start[1]:t_end[1]) 
      target += multi_normal_lpdf(x[t, ] | x[t-1, ], Sigma_x);
