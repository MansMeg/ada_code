...
data {
  ...
  int<lower=0, upper=1> use_sigma_kappa_gamma_prior;
  ...
  real<lower=0> sigma_kappa_gamma_a_hyper;
  real<lower=0> sigma_kappa_gamma_b_hyper;
}
...
parameters {
  ...
  matrix[use_industry_bias ? (T_known + 1) : 0, use_industry_bias ? P : 0] kappa_raw; // Industry bias
  ...
  //jump part
  matrix<lower=0>[T, P] V_noise; 
  vector<lower=2,upper=4>[P] alpha_V; //shape of jumps
  vector<lower=0,upper=1>[P] ar_V; // AR component for jump
  vector<lower=0,upper=1>[P] theta_x; // proportion of jump vs Gauss
  
  //latent ar
  vector<lower=0,upper=1>[P] ar_x; // AR component for jump
...
transformed parameters {
  ...
  // jump process
   matrix<lower=0>[T , P]  V; // unknown states (proportions)
    ...
    for(p in 1:P){
      ...
      if(use_random_walk_kappa) {
        ...
        for(j in 2:(T_known+1)) {
           kappa[j, p] = kappa[j-1, p] + kappa_raw[j, p] * sigma_kappa[p];
        }
      } else {
      ...
      for(i in 1:N) {
        if(y_missing[i, p] == 0){
            ...
            // mu[i,p] = mu[i,p] + gs[i] * kappa_raw[next_known_state_index[i], p] * sigma_kappa[p];
          }
        }
      }
    }
...
model {
  ...
  target += beta_lpdf(theta_x| 1, 1);
  target += normal_lpdf(alpha_V | 2, 10);
  //target += normal_lpdf(ar_V| 0, 2);
  ...
  for(p in 1:P){
    // TODO: Add priors
    target += inv_gamma_lpdf(V_noise[ t_start[p]-1, p]| alpha_V[p],   (alpha_V[p] - 1)*(1-ar_V[p]));
    for(t in t_start[p]:t_end[p]) { 
      target += inv_gamma_lpdf(V_noise[t, p]| alpha_V[p],   (alpha_V[p] - 1)*(1-ar_V[p]));
      target += normal_lpdf(x[t, p] | ar_x[p]*x[t-1, p], sigma_x[p] * ((1-theta_x[p])  * V[t,p]  + theta_x[p]) );
    }
  }
  ...
  if(use_industry_bias){
    if(use_random_walk_kappa) {
      for(p in 1:P){
        ...
        if(use_sigma_kappa_gamma_prior){
          target += gamma_lpdf(sigma_kappa[p] | sigma_kappa_gamma_a_hyper, sigma_kappa_gamma_b_hyper);
        } else {
