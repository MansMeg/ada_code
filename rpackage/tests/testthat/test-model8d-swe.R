...
test_that("test model 8d: Design effect in Sweden 2010-", {
  ...
  # 2: There were 25 transitions after warmup that exceeded the maximum treedepth. Increase max_treedepth above 10. See
  ...
  for(party in parties){
    ...
    expect_silent(
      plt <- plot_poll_of_polls(pop8d, party, from = "2018-01-01") + geom_known_state(pop8d, party) + ggplot2::theme_bw()
    )
  ...
  for(i in seq_along(parties)){
    ...
    for(j in seq_along(houses)){
      ...
      if(print_plt) ggplot2::ggsave(plt, filename = file.path(tmp_folder, paste0("beta_mu_",party,"_",make.names(house),".jpg")), width = 4, height = 3)
    }
  }
